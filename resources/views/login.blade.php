...
<body class="flex justify-end w-screen h-screen min-h-screen bg-login_img bg-cover bg-no-repeat bg-right 
    ...
    <div class="w-2/5 h-full px-10 flex flex-col justify-center items-stretch rounded-s-2xl bg-color-bg shadow-lg 
        max-md:w-full max-md:rounded-xl max-md:px-5 max-md:py-8 max-md:h-fit text-white">
        ...
        <form method="POST" action="{{ route('login.cliente') }}">
            ...
            <a href="{{ route('registrar.cliente') }}" class="text-center text-color-main hover:text-orange-600 hover:underline">¿No tienes una cuenta? Regístrate aquí</a>

            <button type="submit" class=" w-full p-3 mt-5 font-bold text-center uppercase bg-color-main rounded-lg text-white hover:bg-orange-600">Ingresar</button>
        </form>
    </div>
</body>
</html>